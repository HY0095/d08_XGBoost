...
def xgbt_pred(estimator, List, ndim):
  ...
  df = df.transpose()
  colmns = ['f'+str(i) for i in xrange(ndim)]
  df.columns = columns
...
if __name__ == "__main__":
  ...
  parser.add_argument('-pred_model', default='best_model_***.pkl')
  ...
  predRdd = hc.sql(sql).rdd.cache()
  predRdd.map(lambda x: [x[0], xgbt_pred(estimator, x[1], 288)]).toDF(col_name).registerTempTable('xgbt_pred_score_tmp')
